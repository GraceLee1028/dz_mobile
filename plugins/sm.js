    ...
    var Picker = function (params) {
        ...
        var defaults = {
            ...
            toolbarTemplate: '<header class="bar bar-nav">\
                <button class="button button-link button-yes pull-right close-picker">确定</button>\
        ...
        p.updateValue = function () {
            ...
            if (p.input && p.input.length > 0) {
                window.$input = $(p.input);
                window.currentVal = p.params.formatValue ? p.params.formatValue(p, p.value, p.displayValue) : p.value.join(' ');
                $(p.input).attr("data-value",p.currentVal);
                //$(p.input).val(p.currentVal);
        ...
        if(p.params.isYes){
            $(document).off("click").on("click", ".button-yes", function() {
                //p.updateValue(true);
                console.log("当前值"+ window.currentVal);
                console.log(window.$input);
                window.$input.val(window.currentVal);
